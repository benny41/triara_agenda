  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <form method="post" enctype="multipart/form-data" action="{{ route('contacto.store') }}" onsubmit=" return validarFormulario()">
                {{ csrf_field() }}
          ...
          <tbody id="myTable">
            ...
            <tr>
              ...
              <td>
                ...
                <img src="imgs/users/{{$contacto->imagen }}" width="70px" height="70px" ><br> 
                @else
                <img src="imgs/users/default.png" alt="contacto-img" class="img-fluid" width="70px" height="70px">
                @endif
              </td>
              <td>{{$contacto->nombre}} </td>
              <td>{{$contacto->apellido_p}} {{$contacto->apellido_m}}</td>
              <td>{{$contacto->alias}}</td>
              <td>
                <!--<button type="button" class="edit btn btn-primary btn-sm" data-toggle="modal" data-target="#VerModal" >
                <i class="far fa-eye"></i>
                </button>-->
                <a class="btn btn-sm btn-info" href="{{ route('contacto.edit',$contacto->id) }}"><i class="fas fa-edit"></i>
                  <a href="{{route('elicontacto',$contacto->id) }}"><button class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></button></a>
              </td>
            </tr>
            @endforeach
        </tbody>
